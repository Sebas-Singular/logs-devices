...
<header class="border-b border-slate-200 bg-white">
    <div class="mx-auto grid max-w-7xl grid-cols-[auto_1fr_auto] items-center gap-6 px-6 py-5">
        <a href="/" class="flex min-w-0 flex-col items-start gap-1">
            ...
            <div class="text-sm text-slate-500">Logs Devices</div>
        </a>
        ...
        <nav class="flex items-center justify-center gap-3 text-sm">
            <?php foreach ($navItems as $item): ?>
                ...
                <a
                    ...
                    class="<?= $isActive ? 'bg-slate-900 text-white' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900' ?> rounded-lg px-4 py-2 font-medium whitespace-nowrap">
        ...
        <div class="flex justify-end">
            <a href="/logout.php"
                class="rounded-lg px-3 py-2 text-sm font-medium text-slate-500 hover:bg-slate-100 hover:text-slate-900 whitespace-nowrap"
                title="Cerrar sesión">
                Salir
            </a>
        </div>
